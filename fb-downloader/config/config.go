...
var (
	...
	// Facebook Extractor
	FbExtractorURL     = getEnvOrDefault("FB_EXTRACTOR_URL", "http://fb-extractor:8002")
	FbExtractorTimeout = time.Duration(getEnvIntOrDefault("EXTRACT_API_TIMEOUT", 15)) * time.Second

	// Facebook Cookie (from env, no DB)
	FbDefaultCookie = getEnvOrDefault("FB_DEFAULT_COOKIE", "")
