...
func HandlePrepare(c *fiber.Ctx) error {
	...
	// Pick video URL (prefer progressive for combined video+audio)
	...
	go processPrepareJob(jobID, req.URL, videoURL, audioURL)
	...
	if audioURL != "" {
		proxyAudioURL = utils.GenerateMediaProxyURL(audioURL)
	}
...
func processPrepareJob(jobID, postURL, videoURL, audioURL string) {
	...
	_, err := services.Download(ctx, jobID, postURL, videoURL, videoFile)
	if err != nil {
	...
	if audioURL != "" {
		...
		if err != nil {
			...
			fmt.Printf("[Prepare/%s] Audio download failed, extracting from video: %v\n", jobID, err)
			...
			audioPath := filepath.Join(jobDir, audioFile)
			if err := services.RemuxVideo(audioPath, outputAudioPath); err != nil {
				utils.UpdatePrepareMetaError(jobID, fmt.Sprintf("Audio processing failed: %v", err))
				return
			}
		}
	} else {
