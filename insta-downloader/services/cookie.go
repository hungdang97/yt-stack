...
// CookieProvider manages the active Instagram cookie with in-memory cache
...
func (p *CookieProvider) refresh() {
	docs, err := p.repo.GetRandomActiveCookies(10)
	if err != nil {
	...
	if len(docs) == 0 {
		log.Printf("[Cookie] No active cookie in DB — pool is empty\n")
		p.pool = []CookieItem{}
		return
	}
