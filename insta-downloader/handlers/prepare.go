...
func HandlePrepare(c *fiber.Ctx) error {
	...
	if !utils.IsInstagramURL(req.URL) {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{"error": "Invalid Instagram URL"})
	}
...
func processPrepareJob(jobID, postURL, videoURL, audioURL string) {
	...
	if audioURL != "" {
		_, err := services.Download(ctx, jobID, postURL, audioURL, "temp_audio.m4a")
		if err != nil {
			...
			audioPath := filepath.Join(jobDir, "temp_audio.m4a")
