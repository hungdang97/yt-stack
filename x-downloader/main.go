...
func main() {
	log.Println("=== X Downloader Starting ===")
	log.Printf("Port: %d", config.Port)
	log.Printf("Storage: %s", config.StorageDir)
	log.Printf("Extractor: %s", config.XExtractorURL)
	...
	app := fiber.New(fiber.Config{
		AppName:      "X-Downloader",
		ServerHeader: "X-Downloader",
	...
	log.Println("  GET  /proxy/image      - Proxy X/Twitter images")
