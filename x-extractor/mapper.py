"""
Mapper module to convert yt-dlp info into the x-extractor API response.

The response is intentionally aligned with yt-extractor and fb-extractor so that
clients can reuse the same "format picker" logic across platforms. Two views of
the same data are returned:

- ``videoStreams`` / ``audioStreams``: YouTube-style structured format lists.
- ``media``: the legacy progressive/DASH URL shape consumed by x-downloader.
...
def _track_present(codec: Optional[str], ext: Optional[str]) -> bool:
    ...
    Some formats arrive with ``vcodec``/``acodec`` set to ``None`` and only expose
    ``video_ext``/``audio_ext`` (e.g. ``"mp4"`` / ``"none"``). Prefer the codec when
    known, otherwise fall back to the ext hint.
    """
...
def _map_video_stream(fmt: dict) -> dict:
    ...
    if not quality and height:
        quality = f"{height}p"
    if not quality:
        # Fall back to format_id when neither note nor height is available.
...
def build_streams(formats: list[dict]) -> tuple[list[dict], list[dict]]:
    ...
    ``quality`` even when ``height`` is missing).
    """
...
# ============================================
# LEGACY MEDIA SHAPE (consumed by x-downloader)
# ============================================
...
def _best_progressive_url(formats: list[dict]) -> Optional[str]:
    ...
    preserving the proven behaviour x-downloader depends on.
    """
    progressive = [
        f for f in formats
...
def _primary_entry(info: dict) -> dict:
    ...
    For a multi-media tweet, that is the first entry exposing formats; otherwise
    the info object itself is used.
    """
...
def map_x_response(info: dict) -> dict:
    """
    Map a yt-dlp info dict to the x-extractor API response.

    Adds YouTube-style ``videoStreams`` / ``audioStreams`` while preserving the
    legacy ``media`` shape used by x-downloader.
    """
