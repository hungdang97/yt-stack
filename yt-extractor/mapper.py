...
def _pick_vtt_url(tracks):
    """Pick the VTT format URL from a list of subtitle tracks."""
    for t in tracks:
        if t.get('ext') == 'vtt':
            return t.get('url')
...
def _map_subtitles(info):
    ...
    for lang, tracks in (info.get('subtitles') or {}).items():
        if lang == 'live_chat':
            continue
        url = _pick_vtt_url(tracks)
        if url and lang not in seen:
            result.append({'lang': lang, 'url': url, 'autoGenerated': False})
            seen.add(lang)

    # Auto-generated captions (only if no manual for that language)
