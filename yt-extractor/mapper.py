...
def _pick_subtitle_url(tracks, prefer_cdn=False):
    """Pick subtitle URL from tracks. When prefer_cdn=True, prefer googlevideo.com HLS URLs."""
    if prefer_cdn:
        for t in tracks:
            if t.get('ext') == 'vtt' and 'googlevideo.com' in (t.get('url') or ''):
                return t.get('url')
    # Fallback to json3 (easiest to parse on server)
    for t in tracks:
        if t.get('ext') == 'json3':
            return t.get('url')
...
def _map_subtitles(info):
    """
    Map yt-dlp subtitles + automatic_captions to API format.
    Prefers manual subtitles, falls back to auto-generated.
    For auto-generated, prefers googlevideo CDN URLs (avoids 429).
    Returns list of {lang, url, autoGenerated}.
    """
    result = []
    seen = set()

    # Manual subtitles first (no CDN URLs available for manual subs)
    for lang, tracks in (info.get('subtitles') or {}).items():
        if lang == 'live_chat':
            continue
        url = _pick_subtitle_url(tracks, prefer_cdn=False)
        if url and lang not in seen:
            result.append({'lang': lang, 'url': url, 'autoGenerated': False})
            seen.add(lang)

    # Auto-generated captions — only original tracks (skip translated ones with tlang=)
    for lang, tracks in (info.get('automatic_captions') or {}).items():
        if lang in seen or lang == 'live_chat':
            continue
        # Filter out translated tracks: if any URL has tlang=, it's a translation
        if tracks and 'tlang=' in (tracks[0].get('url') or ''):
            continue
        url = _pick_subtitle_url(tracks, prefer_cdn=True)
        if url:
            result.append({'lang': lang, 'url': url, 'autoGenerated': True})
            seen.add(lang)
