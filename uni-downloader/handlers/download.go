...
import (
	...
	"uni-downloader/models"
	"uni-downloader/services"
	"uni-downloader/utils"
...
func HandleDownload(c *fiber.Ctx) error {
	...
	// Block known platform-specific URLs that have dedicated services
	...
	for _, blocked := range blockedDomains {
		if strings.Contains(host, blocked) {
			return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
				"error": fmt.Sprintf("Use the dedicated service for %s", blocked),
			})
		}
	}

	fmt.Printf("[Universal] Download request: %s (type=%s, os=%s)\n", req.URL, req.Type, req.OS)

	// Extract post metadata from uni-extractor
	...
	if title == "" {
		title = postData.Typename + " " + postData.ID
	}
	...
	// Get thumbnail — proxy through our server
