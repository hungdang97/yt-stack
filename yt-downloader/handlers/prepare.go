...
type PrepareResponse struct {
	...
	Subtitle          *SubtitleInfo `json:"subtitle,omitempty"` // sub in video's original audio language
}
...
func HandlePrepare(c *fiber.Ctx) error {
	...
	subtitle := pickOriginalSubtitle(extractData, extractData.Duration)
...
// pickOriginalSubtitle returns the subtitle in the video's original audio
...
//
// Returns nil if no subtitle has a URL.
//
...
func pickOriginalSubtitle(extractData *models.ExtractResponse, duration float64) *SubtitleInfo {
	...
	return &SubtitleInfo{
		Lang:          pick.Lang,
		URL:           utils.GenerateCaptionURL(pick.URL, pick.Lang, duration),
