...
yt-downloader & tik-downloader use gost as HTTP proxy for downloads
```
...
| **nginx** | - | nginx:alpine | 80/443 | SSL termination (Let's Encrypt), rate limiting (30 req/s), routes `/` → yt, `/tik/` → tik |
...
### Go services structure (yt-downloader, tik-downloader, vps-agent)
```
...
- **Nginx routing**: `/` routes to yt-downloader, `/tik/` routes to tik-downloader with path stripping.
