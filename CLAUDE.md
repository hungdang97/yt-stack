...
**download-stack** is a scalable video download system with a Hub-VPS Agent architecture. It handles YouTube, TikTok, Instagram, and Facebook content via multiple microservices deployed together with Docker Compose.
...
yt-downloader, tik-downloader, insta-downloader & fb-downloader use gost as HTTP proxy for downloads
```
...
| **nginx** | - | nginx:alpine | 80/443 | SSL termination (Let's Encrypt), rate limiting (30 req/s), routes `/` → yt, `/tik/` → tik, `/insta/` → insta, `/fb/` → fb |
...
### Go services structure (yt-downloader, tik-downloader, insta-downloader, fb-downloader, vps-agent)
```
...
### Python services (yt-extractor, tik-extractor, insta-extractor, fb-extractor)
...
- **Nginx routing**: `/` routes to yt-downloader, `/tik/` routes to tik-downloader, `/insta/` routes to insta-downloader, `/fb/` routes to fb-downloader, all with path stripping.
- **Hub platform detection**: `proxy.go` maps domain → path prefix (`instagram.com` → `/insta`, `tiktok.com` → `/tik`, `facebook.com` → `/fb`, default → YouTube).
